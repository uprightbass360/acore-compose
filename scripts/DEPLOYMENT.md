...
This document describes how to use the universal deployment script for the AzerothCore Docker stack. The script supports both custom environment files (generated by setup wizard) and original template files for backward compatibility.
...
### Dry Run (Test Configuration Without Deployment)
```bash
cd scripts
...
| Option | Description |
|--------|-------------|
| `--cleanup` | Clean up existing deployment before starting |
| `--skip-wizard` | Skip setup wizard (use existing env files) |
| `--use-templates` | Use original template env files instead of custom ones |
...
All URLs and ports are dynamically configured based on your environment files. The script will display the actual URLs and credentials after deployment.

### Default URLs (Template Mode)
- 🌐 **PHPMyAdmin**: http://localhost:8081
- 🛠️ **Keira3**: http://localhost:4201

### Default Game Connections (Template Mode)
- 🎮 **Game Server**: localhost:8215
- 🔐 **Auth Server**: localhost:3784
- 🔧 **SOAP API**: localhost:7778
- 🗄️ **MySQL**: localhost:64306

### Default Credentials (Template Mode)
- **MySQL**: root / azerothcore123

**Note**: Custom environment files may use different ports and credentials. Check the deployment summary for actual values.
